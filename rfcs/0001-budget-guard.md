...
- Authors: @ziro-ai/maintainers
- Status: draft
- Affected packages: `@ziro-ai/core`, `@ziro-ai/agent`, `@ziro-ai/tools`, `@ziro-ai/gateway`
...
This requires per-provider pricing tables, kept in `@ziro-ai/core/pricing` and updated weekly via a scheduled CI job (see also: `pricing-table.json` schema in §Adoption).
...
```ts
import { getCurrentBudget } from '@ziro-ai/core';
...
These integrate with the standard `@ziro-ai/tracing` exporter and are visible in the playground's trace timeline.
...
| --- | --- | --- |
| v0.1.0 | `@ziro-ai/core` | `BudgetSpec`, `BudgetExceededError`, `withBudget`, `generateText({ budget })` |
| v0.1.0 | `@ziro-ai/openai`, `@ziro-ai/anthropic` | `estimateCost()` + pricing tables |
| v0.1.1 | `@ziro-ai/agent` | `agent.run({ budget, toolBudget })`, AsyncLocalStorage context |
| v0.1.2 | `@ziro-ai/tools` | `defineTool({ budget })`, `getCurrentBudget()` helper |
| v0.1.x | `@ziro-ai/tracing` | OTel events `ziro.budget.*` |
| v0.2.0 | `@ziro-ai/eval` | `cost-budget` grader for eval suite |
| v0.2.0 | `@ziro-ai/temporal`, `@ziro-ai/inngest` | Budget state persistence on crash/resume |
| v0.3.0 | `@ziro-ai/gateway` | Per-virtual-key budget enforcement |

### Pricing-table format (`@ziro-ai/core/pricing`)
