 ...
 * `@ziro-agent/checkpoint-postgres` to keep this package zero-runtime-dep
 * (we don't take a dep on the postgres adapter just for one helper).
 *
 ...
 * `ORDER BY created_at` on every read path. For Redis specifically, we
 * additionally store the millisecond timestamp as the sorted-set score
 * so range queries by time work without parsing the id.
 */
